...
TEST_CASE("layer1 eval should not put top level let statements into the root") {
    ...
    CHECK(std::holds_alternative<Expression*>((*x->body_)->get_value()));
    auto expression = std::get<Expression*>((*x->body_)->get_value());
...
//     auto expr = std::get<Expression*>((*result.top_level_definition)->get_value());
//     CHECK(expr->expression_type == ExpressionType::layer2_expression);
