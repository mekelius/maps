...
TEST_CASE("Should be able to create a non-existent function type based on signature") {
    ...
    SUBCASE("nullary function") {
        auto function_type = types.get_function_type(&TestingType, array<const Type*, 0>{}, true);
    ...
    SUBCASE("binary function") {
        auto function_type = types.get_function_type(&TestingType, array{&TestingType, &TestingType}, true);
